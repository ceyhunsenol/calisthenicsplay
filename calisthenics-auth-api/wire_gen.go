...
func InitializeApp() *echo.Echo {
	...
	authController := api.NewAuthController(iAuthService)
	iProfileRepository := repository.NewProfileRepository(db)
	iProfileService := service.NewProfileService(iProfileRepository)
	iUserProfileService := service.NewUserProfileService(iProfileService)
	userProfileController := api.NewUserProfileController(iUserProfileService)
	userController := api.NewUserController(iUserService)
	echoEcho := InitRoutes(authController, userProfileController, userController)
	return echoEcho
}
...
var DomainServiceSet = wire.NewSet(service.NewUserService, service.NewProfileService)

var ServiceSet = wire.NewSet(service.NewAuthService, service.NewUserProfileService)

var ControllerSet = wire.NewSet(api.NewAuthController, api.NewUserProfileController, api.NewUserController)
