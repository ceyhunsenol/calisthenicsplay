...
func (g *GenreTypeController) SaveGenreType(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, &MessageResource{Message: "GenreType could not be saved."})
	}
	if exists {
		return c.JSON(http.StatusBadRequest, &MessageResource{Message: "GenreType already exists in this code."})
	}
...
func (g *GenreTypeController) UpdateGenreType(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusNotFound, &MessageResource{Message: "GenreType not found."})
	}
	genreType.Code = genreTypeDTO.Code
	_, err = g.genreTypeService.Update(*genreType)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, &MessageResource{Message: "GenreTypes could not be updated."})
	}
...
func (g *GenreTypeController) GetGenreTypes(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, &MessageResource{Message: "GenreTypes could not be got."})
	}
...
func (g *GenreTypeController) DeleteGenreType(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, &MessageResource{Message: "GenreType could not be deleted."})
	}
